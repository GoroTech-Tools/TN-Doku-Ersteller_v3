# Build-Skript für TN-Doku-Ersteller-Portable
...
$distPath   = Join-Path $PSScriptRoot "dist\$folderName"
$oneFileExePath = Join-Path $PSScriptRoot 'dist\TN-Doku-Ersteller-Portable.exe'
...
if ($Quiet) {
    & py -m PyInstaller TN-Doku-Ersteller-Portable.spec --noconfirm *> $pyInstallerLog
} else {
    & py -m PyInstaller TN-Doku-Ersteller-Portable.spec --noconfirm
}
...
# _Listen (Template-Ordner)
$listenSrc = Join-Path $PSScriptRoot '_Listen'
if (Test-Path $listenSrc) {
    $listenDest = Join-Path $distPath '_Listen'
    Copy-Item -Path $listenSrc -Destination $listenDest -Recurse -Force
    Write-Host "  _Listen kopiert." -ForegroundColor DarkGray
} else {
    Write-Host "  Hinweis: _Listen-Ordner nicht gefunden - wird nicht mitkopiert." -ForegroundColor Yellow
}

# Teilnehmer-CSV (Beispiel/Vorlage)
$csvSrc = Join-Path $PSScriptRoot 'Teilnehmer_Beginn.CSV'
if (Test-Path $csvSrc) {
    Copy-Item -Path $csvSrc -Destination (Join-Path $distPath 'Teilnehmer_Beginn.CSV') -Force
    Write-Host "  Teilnehmer_Beginn.CSV kopiert." -ForegroundColor DarkGray
}
